...
Для начала вспомним, как работает обычный градиентный спуск. Пусть объекты задаются n числовыми признаками![Image alt](https://github.com/Ragnarok7861/Victor/blob/master/FS.png)
    и пространство признаковых описаний в таком случае ![Image alt](https://github.com/Ragnarok7861/Victor/blob/master/FSS.png)
Пусть Y − конечное множество меток классов и задана обучающая выборка пар «объект-ответ» ![Image alt](https://github.com/Ragnarok7861/Victor/blob/master/pss.png)
 . Пусть семейство алгоритмов a(x,w) имеет параметр вектор весов w. И пускай мы выбрали какую-нибудь функцию потерь. Для i-го объекта выборки для алгоритма с весами w обозначим ее Li(w). Необходимо минимизировать эмпирический риск, т.е. ![Image alt](https://github.com/Ragnarok7861/Victor/blob/master/png)
. Если функция потерь принадлежит классу C1(X), то можно применить метод градиентного спуска. Выберем w(0) − начальное приближение. Тогда каждый следующий вектор параметров будет вычисляться как ![Image alt](https://github.com/Ragnarok7861/Victor/blob/master/bs.png)
 , где h - градиентный шаг, смысл которого заключается в том, насколько сильно менять вектор весов в направлении градиента. Остановка алгоритма будет определяться сходимостью Q или w.

# SGD

Проблема предыдущего алгоритма заключается в том, что чтобы определить новое приближение вектора весов необходимо вычислить градиент от каждого элемента выборки, что может сильно замедлять алгоритм. Идея ускорения алгоритма заключается в использовании только одного элемента, либо некоторой подвыборки для подсчета нового приближения весов. То есть теперь новое приближение будет вычисляться как w(t+1)=w(t)−h∇Li(w(t)), где i − случайно выбранный индекс. Так как теперь направление изменения w будет определяться за O(1), подсчет Q на каждом шаге будет слишком дорогостоящим. Для того, чтобы ускорить оценку Q, будем использовать приближенную рекуррентную формулу. Можно выбрать одну из следующих формул:

среднее арифметическое: Q¯¯¯¯m=1mεm+1mεm−1+1mεm−2+⋯=1mεm+(1−1m)Q¯¯¯¯m−1;
экспоненциальное скользящее среднее: Q¯¯¯¯m=λεm+(1−λ)εm−1+(1−λ)2εm−2+⋯=λεm+(1−λ)Q¯¯¯¯m−1, где λ − темп забывания предыстории 
