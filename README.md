...
Пусть Y − конечное множество меток классов и задана обучающая выборка пар «объект-ответ» ![Image alt](https://github.com/Ragnarok7861/Victor/blob/master/pss.png)
 . Пусть семейство алгоритмов a(x,w) имеет параметр вектор весов w. И пускай мы выбрали какую-нибудь функцию потерь. Для i-го объекта выборки для алгоритма с весами w обозначим ее Li(w). Необходимо минимизировать эмпирический риск, т.е. Q(w)=∑i=1lLi(w)→minw. Если функция потерь принадлежит классу C1(X), то можно применить метод градиентного спуска. Выберем w(0) − начальное приближение. Тогда каждый следующий вектор параметров будет вычисляться как w(t+1)=w(t)−h∑i=1l∇Li(w(t)), где h - градиентный шаг, смысл которого заключается в том, насколько сильно менять вектор весов в направлении градиента. Остановка алгоритма будет определяться сходимостью Q или w.
